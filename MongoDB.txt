...
//4.
db.products.find({product_price:{$gte:400}}).limit(4).pretty()
...
//10.
db.products.deleteMany({ "product_price": { "$in": db.products.aggregate([{ "$group": { "_id": "$product_price", "count": { "$sum": 1 } } }, { "$match": { "count": { "$gt": 1 } } }]).map(doc => doc._id) } })